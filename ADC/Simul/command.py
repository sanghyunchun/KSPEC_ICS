...
async def identify_excute(ADC_server,adc_action,cmd):    # For real observation
    ...
    if func == 'adcadjust':
        ...
        asyncio.create_task(handle_adcadjust(ADC_server,adc_action,ra,dec,obsnum))


async def handle_adcadjust(ADC_server,adc_action,ra,dec,obsnum):
...
def calculate_zenith_distance(ra_obj,dec_obj):
    ...
    times = current_time + np.arange(0, 6) * u.minute    # every 1 minute for 5 min.
