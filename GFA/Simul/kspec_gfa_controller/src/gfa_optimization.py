    ...
    for ipd in ipd_list:
        for ftd_base in ftd_base_list:
            ...
            for trial in range(1, repeat+1):
                ...
                # timeout 카메라 집계
                timeout_cams = [cam for sub in res if isinstance(sub, list) for cam in sub]
...
async def main():
    ...
    try:
        ...
        best_combinations = df_no_timeout.sort_values(by=["Camera", "Avg_FPS"], ascending=[True, False]) \
                                         .groupby("Camera").head(1)
        best_combinations.to_csv("best_combinations_per_camera.csv", index=False)
        print("[SAVED] 카메라별 최적 조합이 'best_combinations_per_camera.csv'에 저장되었습니다.")

        # Timeout 발생 조합도 별도 저장
        unstable = df[df["Timeout_Count"] > 0]
        unstable.to_csv("unstable_combinations.csv", index=False)
        print("[SAVED] Timeout 발생 조합이 'unstable_combinations.csv'에 저장되었습니다.")
