...
class GFAGuider:
    ...
    def select_stars(
        ...
        mask = (
            (angular_distance_degrees < self.ang_dist) &
            (valid_flux > mag_flux_min)
        )
    ...
    def cal_centroid_offset(
        ...
        """
        Calculate centroid offsets for each guide star by finding peaks in cutouts.

        Parameters
        ----------
        dra, ddec : np.ndarray
            Integer X, Y positions of stars.
        dra_f, ddec_f : np.ndarray
            Floating-point X, Y positions of stars.
        stddev : float
            Background standard deviation from `background()`.
        wcs : WCS
            The WCS object for converting pixel offsets to arcseconds.
        fluxn : np.ndarray
            Flux values of the selected stars.
        file_counter : int
            An index used for naming saved FITS cutouts.
        cutoutn_stack : list of np.ndarray
            A list where cutouts for max-flux stars can be appended.
        image_data : np.ndarray
        ...
        -------
        dx, dy : list of float
            X, Y offsets in arcseconds for each star.
        peakc : list of float
            Peak values for each star.
        cutoutn_stack : list of np.ndarray
            The updated stack of saved cutouts.
        """
        self.logger.debug("Calculating centroid offsets for guide stars.")
        dx, dy, peakc = [], [], []
        ...
        for i in range(len(dra)):
            try:
                ...
                # Calculate the centroid in cutout2
                xcs, ycs = 0.0, 0.0
                ...
                dy.append(0)
                peakc.append(-1)
                self.logger.warning(f"Error finding peaks: {exc}")

        self.logger.debug("Centroid offset calculation completed.")
    ...
    def exe_cal(self) -> Tuple[float, float, float]:
        """
        Execute the main guide star calibration routine:
          1. Load astrometry and processed images.
          2. For each pair of files, compute background, load star catalog,
             select stars, compute offsets, and gather results.
          3. Compute final offset and FWHM.

        Returns
        -------
        tuple of float
            (fdx, fdy, fwhm), each in arcseconds, or NaN if failure.
        """
        ...
        if not astro_dir or not proc_dir:
            self.logger.error("Astrometry or processed directory path missing in configuration.")
        ...
        if not astro_files:
            self.logger.error(f"No astrometry FITS files found in {astro_dir}. Calibration aborted.")
            return math.nan, math.nan, math.nan
        if not proc_files:
            self.logger.error(f"No processed FITS files found in {proc_dir}. Calibration aborted.")
            return math.nan, math.nan, math.nan

        self.logger.info(
            f"Found {len(astro_files)} astrometry files and "
            f"{len(proc_files)} processed files."
        )
        ...
        for astro_file, proc_file in zip(astro_files, proc_files):
            try:
                self.logger.info(f"Processing FITS: {astro_file} & {proc_file}")

                # Load astrometry file + WCS
                ...
                # Star catalog and selection
                (ra1_rad, dec1_rad,
                 ra2_rad, dec2_rad,
                 ra_p, dec_p, flux) = self.load_star_catalog(crval1, crval2)
                ...
                # Compute offsets
                (dx_vals, dy_vals,
                 peak_vals, cutoutn_stack) = self.cal_centroid_offset(
                     dra, ddec, dra_f, ddec_f,
                     stddev, wcs_obj,
                     flux_sel, file_counter,
                     cutoutn_stack, image_data
                 )
                file_counter += 1

                # Filter by peak thresholds
                ...
                self.logger.error(f"Error processing {astro_file} & {proc_file}: {exc}")
                raise RuntimeError(
        ...
        # Compute FWHM from stacked cutouts
        fwhm = self.cal_seeing(cutoutn_stack)
        self.logger.info(f"FWHM: {fwhm:.2f} arcsec")

        self.logger.info(
            f"Guide star calibration complete. Offsets => "
            f"fdx={fdx:.5f}, fdy={fdy:.5f}, FWHM={fwhm:.5f}"
        )
        return fdx, fdy, fwhm
